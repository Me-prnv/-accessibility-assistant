...
export function createCognitiveAssistant(options = {}) {
    ...
    let simplifiedPageContainer = null;
    ...
    function updateControlPanel() {
        const focusModeCheckbox = document.getElementById('accessibility-assistant-focus-mode');
        if (focusModeCheckbox) {
            focusModeCheckbox.checked = settings.focusModeEnabled;
        }
        const readingGuideCheckbox = document.getElementById('accessibility-assistant-reading-guide');
        if (readingGuideCheckbox) {
            readingGuideCheckbox.checked = settings.readingGuideEnabled;
        }
        const simplifiedPageCheckbox = document.getElementById('accessibility-assistant-simplified-page');
        if (simplifiedPageCheckbox) {
            simplifiedPageCheckbox.checked = settings.simplifiedPageEnabled;
        }
        const readingAidsCheckbox = document.getElementById('accessibility-assistant-reading-aids');
        if (readingAidsCheckbox) {
            readingAidsCheckbox.checked = settings.readingAidsEnabled;
        }
        const highlightLinksCheckbox = document.getElementById('accessibility-assistant-highlight-links');
        if (highlightLinksCheckbox) {
            highlightLinksCheckbox.checked = settings.highlightLinksEnabled;
        }
        const textToSpeechCheckbox = document.getElementById('accessibility-assistant-text-to-speech');
        if (textToSpeechCheckbox) {
            textToSpeechCheckbox.checked = settings.textToSpeechEnabled;
        }
        const vocabularyCheckbox = document.getElementById('accessibility-assistant-vocabulary');
        if (vocabularyCheckbox) {
            vocabularyCheckbox.checked = settings.vocabularySimplificationEnabled;
        }
        const readingSpeedSlider = document.getElementById('accessibility-assistant-reading-speed');
    ...
    function enableFocusMode() {
        if (isFocusModeActive || !focusModeOverlay)
            return;
    ...
    function enableReadingGuide() {
        if (isReadingGuideActive || !readingGuide)
            return;
        // Show reading guide
    ...
    function createSimplifiedPage(summary) {
    ...
    function disableSimplifiedPage() {
        if (!isSimplifiedPageActive || !simplifiedPageContainer)
            return;
